...
if selected == "Home":
    st.image("PHPE.png")
    st.markdown("# :orange[Data Visualization and Exploration]")
    st.markdown("## :orange[A User-Friendly Tool Using Streamlit and Plotly]")
...
# Top Charts - USERS          
    if Type == "Users":
        ...
        with col2:
            st.markdown("### :orange[District]")
            mycursor.execute(f"select district, sum(Registered_Users) as Total_Users, sum(AppOpens) as Total_Appopens from map_user where year = {Year} and quarter = {Quarter} group by district order by Total_Users desc limit 10")
        ...
        with col4:
            st.markdown("### :orange[State]")
            mycursor.execute(f"select state, sum(Registered_users) as Total_Users, sum(AppOpens) as Total_Appopens from map_user where year = {Year} and quarter = {Quarter} group by state order by Total_Users desc limit 10")
...
# EXPLORE DATA - TRANSACTIONS
    if Type == "Transactions":
        ...
        with col1:
            st.markdown("## :orange[Overall State Data - Transactions Amount]")
        ...
        with col2:
            ...
            st.markdown("## :orange[Overall State Data - Transactions Count]")
            mycursor.execute(f"select state, sum(count) as Total_Transactions, sum(amount) as Total_amount from map_trans where year = {Year} and quarter = {Quarter} group by state order by state")
            df1 = pd.DataFrame(mycursor.fetchall(),columns= ['State', 'Total_Transactions', 'Total_amount'])
            df2 = pd.read_csv('Statenames.csv')
...
# BAR CHART TRANSACTIONS - DISTRICT WISE DATA            
        ...
        st.markdown("# ")
        st.markdown("# ")
        st.markdown("## :orange[Select any State to explore more]")
...
# EXPLORE DATA - USERS      
    if Type == "Users":
        ...
        st.markdown("## :orange[Overall State Data - User App opening frequency]")
        mycursor.execute(f"select state, sum(Registered_users) as Total_Users, sum(AppOpens) as Total_Appopens from map_user where year = {Year} and quarter = {Quarter} group by state order by state")
        ...
        mycursor.execute(f"select State,year,quarter,District,sum(Registered_users) as Total_Users, sum(AppOpens) as Total_Appopens from map_user where year = {Year} and quarter = {Quarter} and state = '{selected_state}' group by State, District,year,quarter order by state,district")
